...
| Regulator mapping | Known regulator/domain relationships | Conservative local mapping | Not a complete global registry and not a legal determination |
...
Absence from a feed is not evidence of legitimacy. RDAP privacy-proxy detection is heuristic and may be incomplete. Regulator coverage is geographically and institutionally limited. The score is a triage instrument whose validity must be established on a labelled evaluation set before it is used for consequential decisions.
